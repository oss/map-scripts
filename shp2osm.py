...
__version__ = "$Id: polyshp2osm-Area.py,v 1.7 2009/04/21 00:54:09 tbook Exp tbook $"
...
def ftype(data):
    ...
    waterway = {
        'StreamRiver': 'riverbank',
        'CanalDitch': 'riverbank',
        'Rapids': 'rapids',
        'DamWeir': 'dam'
    }
    whitewater = {
    ...
    if 'ftype' in data:
        keys = []
        if data['ftype'] in waterway:
            keys.append( ('waterway', waterway[data['ftype']]) )
        if data['ftype'] in whitewater:
            keys.append( ('whitewater', whitewater[data['ftype']]) )
        return keys
    return None
...
def fcode(data):
    ...
    if 'fcode' in data:
        keys = []
        if data['fcode'] in water:
            keys.append( ('water', water[data['fcode']]) )
        return keys
    return None
...
def add_ring_nodes(ring):
    ...
    for count in range(ring.GetPointCount() - 1):
        ...
        print >>open_file, "<node id='-%s' lon='%s' lat='%s' />" % (id_counter, ring.GetX(count), ring.GetY(count))
        id_counter += 1
